...
class VisorRutas:
    ...
    def iniciar_busqueda(self):
        #reset de busquedas anteriores
        self.motor.limpiar_solo_rutas()
        
        #encender cronometro de alta precision
        inicio_tiempo = time.perf_counter()
        
        #invocar algoritmo
        ruta = self.buscador.ejecutar_a_estrella()
        ...
        if ruta:
            #procesar el camino omitiendo bordes
            for x, y in ruta[1:-1]:
                self.motor.matriz[y][x] = CAMINO_TRAZADO
            self.dibujar_cuadricula()
            self.label_tiempo.configure(text=f"exito. tiempo: {tiempo_ms:.3f} ms", text_color="#28a745")
        else:
            self.dibujar_cuadricula()
            self.label_tiempo.configure(text=f"sin ruta. tiempo: {tiempo_ms:.3f} ms", text_color="#d9534f")

    def ejecutar_generador_dfs(self):
        self.buscador.generar_laberinto_dfs()
        self.dibujar_cuadricula()
        self.label_tiempo.configure(text="tiempo de ejecucion: 0.000 ms", text_color="#00bfff")

    def limpiar_todo(self):
        #purga desde el motor
        self.motor.limpiar_todo_el_mapa()
        self.dibujar_cuadricula()

    def arrancar(self):
        #mantener aplicacion con vida
        self.ventana.mainloop()

if __name__ == "__main__":
    #instanciar el modelo (30x20 celdas)
    motor_principal = MotorMapa(27, 19)
    
    #instanciar el cerebro
    buscador_rutas = BuscadorRutas(motor_principal)
    
    #instanciar la vista
    app = VisorRutas(motor_principal, buscador_rutas)
    app.arrancar()